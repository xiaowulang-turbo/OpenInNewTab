...
-   **重要更新说明**：发布者可为重要版本启用极简更新说明页；用户可在设置中关闭后续提醒
...
只有发布者明确标记的版本才会打开极简更新说明页。用户可以在选项页关闭或重新开启后续提醒。静默版本会在后台完成更新，不会打开新标签页。
...
如需在本地测试更新说明，可临时将 `update-notices.js` 中当前版本的
`showUpdateNotice` 设为 `true`，补齐双语说明内容，再将扩展以更高的清单版本安装为更新。测试后恢复为静默配置。更新说明提醒也可以在选项页重新开启。
...
├── link-policy.js         # 主机 / 点击策略（经典脚本，测试共用）
├── update-notices.js      # 版本化更新说明数据与校验
...
-   **link-policy.js**：主机与点击跳过规则；在 content.js 之前注入
